...
{
  let array = ["w", "a", "s", "d", 119, 97, 115, 100];
  function randomX() {
    const min = 0;
    const max = 7;
    return Math.floor(Math.random() * (max - min)) + min; //return Math.floor(Math.random() * array.length); другой способ
  } //ф-ция для рандома по кол-ву ячеек массива
  let direction = array[randomX()]; //реализация выбора рандомной ячейки массива

  if (direction === "w" || direction === 119) {
    console.log("Вверх");
  } //тут || это ИЛИ
  if (direction === "a" || direction === 97) {
    console.log("Влево");
  }
  if (direction === "s" || direction === 115) {
    console.log("Вправо");
  }
  if (direction === "d" || direction === 100) {
    console.log("Вниз");
  }
}
